...
from ddi_scorer.pipeline import enrich, filter_twosides, load_dental_drug_set, load_dental_drug_lookup
...
def test_filter_empty_result_when_no_dental_match(tmp_path):
    ...
    df = pd.DataFrame({
        ...
        "PRR": [1.1], "PRR_error": [0.2], "mean_reporting_frequency": [0.01],
        "A": [5], "B": [200], "C": [10], "D": [2000],
        "drug_1_rxnorn_id": ["1"], "drug_2_rxnorm_id": ["2"],
...
def dental_lookup():
    return {
        "amoxicillin": {"category": "Antibiotic", "display_name": "Amoxicillin", "atc_code": "J01CA04", "notes": ""},
        "ibuprofen":   {"category": "NSAID Analgesic", "display_name": "Ibuprofen", "atc_code": "M01AE01", "notes": ""},
    }

...
SEVERITY_RULES: list[tuple[float, list[str]]] = [
    ...
    (1.0, [
        "death", "cardiac arrest", "ventricular fibrill", "ventricular tachycard",
        "qt prolongation", "torsade", "torsades",
        "rhabdomyolysis",
    ...
    (0.5, [
        ...
        "thrombocytopenia", "neutropenia", "leucopenia", "leukopenia",
        "myopathy", "myalgia",
        "neuropathy", "peripheral neuropathy",
        "arrhythmia", "atrial fibrill", "bradycardia", "tachycardia",
        "syncope", "loss of consciousness",
        "hypoglycaemia", "hypoglycemia",
        "hyponatraemia", "hyponatremia",
        "hallucination", "psychosis", "confusion",
        "renal impairment", "creatinine increased",
        "hypotension",
        "pancreatitis",
...
def aggregate_scores(df: pd.DataFrame) -> pd.DataFrame:
    ...
    *relative* risk within this dental-drug population rather than
    ...
    agg["prr_component"] = (
        rankdata(agg["prr_max"].fillna(0), method="average") / n
    )

    # Breadth component: rank by number of distinct adverse effects.
    agg["breadth_component"] = (
        rankdata(agg["n_adverse_effects"], method="average") / n
    )
    ...
    agg["severity_component"] = (
        (agg["mean_severity_weight"] - sw_min) / (sw_max - sw_min)
    ).fillna(0)
...
def add_top_adverse_effects(df_agg: pd.DataFrame, df_full: pd.DataFrame) -> pd.DataFrame:
    ...
    top = (
        df_full
        .sort_values(["severity_weight", "PRR"], ascending=[False, False])
...
def print_summary(df: pd.DataFrame) -> None:
    ...
    top10 = df.nlargest(10, "risk_score")[
        ["dental_drug_name", "patient_drug_name", "risk_score",
         "severity_bin", "n_adverse_effects", "prr_max"]
    ]
    for _, row in top10.iterrows():
        log.info(
            "  %-20s ↔ %-20s  score=%-5.1f  bin=%-8s  n_ae=%-5d  prr_max=%.1f",
            row["dental_drug_name"], row["patient_drug_name"],
            row["risk_score"], row["severity_bin"],
            row["n_adverse_effects"], row["prr_max"] if not np.isnan(row["prr_max"]) else 0,
        )
